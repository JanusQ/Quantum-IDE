...
import { addNoticeAdmin, deleteNoticeAdmin, getNoticeList, updateNoticeAdmin } from '../../api/notice'
	...
	const columns = [
		...
		},
		{
			...
			render: (text, record) => {
				return (
					<span>
						<a style={{ marginRight: '10px' }} onClick={() => edit(record.notice_id)}>
							编辑
						</a>
		...
		return (
			<Modal visible={isAddModalVisible} onCancel={addCancel} onOk={addOk} title='添加公告' width={820}>
				<Form form={addForm} layout='vertical'>
					...
					<Form.Item label='公告上传' name='file' rules={[{ required: true, message: '请上传公告' }]}>
						<Upload
							multiple={false}
							fileList={fileList}
	...
	return (
		<AdminLayout>
			...
			<div className='admin_notice_div'>
				<Table columns={columns} dataSource={noticeList} pagination={false} rowKey='notice_id' />
			</div>
			{addModal()}
		</AdminLayout>
	)
