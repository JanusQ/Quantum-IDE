...
export default class QCEngine {
  ...
  parseBinaryQubits(binary_qubits) {
    if (binary_qubits != undefined) return binary2qubit1(binary_qubits)
    else {
  ...
  getBinaryValue(qubit_set) {
    ...
    while (i < this.qubit_number) {
      if (qubit_set.includes(i)) value.push(1)
      else value.push(0)
      i++
    }
    return value
  ...
  qprint() {
    ...
    // debugger
    this.console_data.push([...arguments].map((elm) => String(elm)).join('  '))
  }

  // TODO: 判断所有控制门的比特会不会重叠，重叠报错
  checkOverlap(controls, targets) {
    for (let ele in targets) {
      if (controls.includes(ele)) return true
    }
    return false
  }
        ...
        if (gates[j][i] != null) {
          ...
          if (control_set.includes(nam)) {
            ...
            for (let k = 0; k < qlen; k++) {
              if (
                gates[k][i] != null &&
                control_set.includes(gates[k][i]['name'])
              ) {
  ...
  _toArray(wires) {
    ...
    } else if (typeof wires == 'number') wires = [wires]
    else if (Array.isArray(wires)) wires = wires
    else {
    ...
    if (pars != undefined) {
      for (let k in pars) {
        if (k == 'phi' || k == 'theta' || k == 'lambda')
          if (pars[k] !== 0) {
            pars[k] =
              pars[k] > 0 ? 'pi/' + 180 / pars[k] : '-pi/' + 180 / -pars[k]
          }
      }
  ...
  exchange(qubits1, qubits2) {
    ...
    if (qubits1.length != qubits2.length) {
      console.error(
        qubits1,
        qubits2,
        'do not have same length, which can not be swapped'
      )
      debugger
    }
  ...
  getQubit2Variable(qubit) {
    ...
    let corresponding_variable = undefined,
      corresponding_index = undefined
  ...
  getLabelUpDown(label_id) {
    ...
    range(start_operation, end_operation).forEach((index) => {
      ...
      qubits_involved = [
        ...qubits_involved,
        ...this.getQubitsInvolved(operations[index]),
      ]
    })
    // console.log(qubits_involved)
    qubits_involved = [...new Set(qubits_involved)]

    let down_qubit = Math.max(...qubits_involved),
      up_qubit = Math.min(...qubits_involved)
    let down_varable = this.getQubit2Variable(down_qubit).variable,
      up_varable = this.getQubit2Variable(up_qubit).variable
  ...
  // TODO: 统一驼峰还是下划线命名
  setState(
    state_vector //set initial state for test, invocate it before adding other gates
  ) {
    ...
    if (opera) {
      ...
      // TODO用不了现在
      old_state = range(0, Math.pow(2, qubit_number)).map((base) =>
        complex(0, 0)
      )
  ...
  getVarState(operation_index, filter = undefined) {
    ...
    for (let key in var_index) {
      ...
      for (let i = 0; i < Math.pow(2, bits); i++) {
        ...
        res[key]['prob'][i] = average(
          whole['probs'],
          tmp_index,
          whole['probs'],
          'probs'
        )
        res[key]['magn'][i] = average(
          whole['probs'],
          tmp_index,
          whole['probs'],
          'magns'
        )
        if (1 - res[key]['magn'][i] < 1e-5) res[key]['magn'][i] = 1.0
        res[key]['phase'][i] = average(
          whole['phases'],
          tmp_index,
          whole['probs'],
          'phases'
  ...
  getIndex(operation_index, filter) {
    ...
    neo_sv = neo_sv.sort(function (a, b) {
      return a - b
    })
    return neo_sv
  }
  ...
  _getPartialTrace(name, operation_index) {
    ...
    for (
      now_num = 0;
      now_num < Math.pow(2, this.qubit_number - bits);
      now_num++
    ) {
      ...
      for (let i = 0; i < ids.length; i++) {
        ...
        vecs[i] = complex(
          state[ids[i]]['amplitude'].re,
          state[ids[i]]['amplitude'].im
        )
      }
      ...
      if (fin_den == undefined) fin_den = math.multiply(den, prob)
      else fin_den = math.add(fin_den, math.multiply(den, prob))
    }
  ...
  getEntropy(operation_index, precision = 1e-5, type = 'qubit') {
    ...
    if (Math.abs(ent - 0) < precision) ent = 0
          ...
          for (i = 0; i < len; i++)
            for (j = 0; j < len2; j++) {
              if (key != key2) {
                ...
                if (Math.abs(pmi - 0) < precision) pmi = 0
                // if(pmi != 0){
  ...
  _variableFilter(operation_index, target, filter) {
    ...
    for (
      let i = 0;
      i < Math.pow(2, var_index[target][1] - var_index[target][0]);
      i++
    ) {
  ...
  _makeState(label_id, status) {
    ...
    let ops = [
      this.labels[label_id]['start_operation'],
      this.labels[label_id]['end_operation'],
    ]

    let op_index
    if (status == 'start') op_index = ops[0] - 1
    else if (status == 'end') op_index = ops[1] - 1
    ...
    for (let key in var_index) {
      if (tmp_array.includes(key)) vars.push(key)
    }
    ...
    for (let i = 0; i < deep_length; i++) {
      ...
      for (let k = 0; k < vars.length; k++) {
        let tmp = bin.slice(
          new_var_index[vars[k]][0],
          new_var_index[vars[k]][1]
        )
        tmp = tmp.reverse()
      ...
      input_state['bases'][i]['magnitude'] = average(
        whole['probs'],
        tmp_index,
        whole['probs'],
        'magns'
      )
      input_state['bases'][i]['phases'] = average(
        whole['phases'],
        tmp_index,
        whole['probs'],
        'phases'
      ...
      for (let k = 0; k < Math.pow(2, this.qubit_number - qubit_num); k++) {
        ...
        for (let key in var_index) {
          ...
          bin = bin.reverse()
          input_state['bases'][i]['related_bases'][k]['var2value'][key] =
            binary2int(bin)
        }
        ...
        for (let key in var_index) {
          ...
          input_state['bases'][i]['related_bases'][k]['range'][key] =
            Math.pow(2, var_bits) - 1
        }
        ...
        input_state['bases'][i]['related_bases'][k]['magnitude'] =
          whole['magns'][order]
        input_state['bases'][i]['related_bases'][k]['phases'] =
          whole['phases'][order]
    ...
    for (let i = 0; i < input_state['bases'].length; i++) {
      if (input_state['max_magn'] == 0) {
        for (
          let j = 0;
          j < input_state['bases'][i]['related_bases'].length;
          j++
        ) {
        ...
        for (
          ...
          input_state['bases'][i]['related_bases'][j]['ratio'] =
            input_state['bases'][i]['related_bases'][j]['magnitude'] /
            input_state['max_magn']
        }
        input_state['bases'][i]['ratio'] =
          input_state['bases'][i]['magnitude'] / input_state['max_magn']
      }
    }
  ...
  canShow(label_id) {
    ...
    for (let i = ops[0]; i < ops[1]; i++) {
      let opera = this.operations[i]
      if (
        opera['operation'] == 'write' ||
        opera['operation'] == 'read' ||
        this.dont_draw_evo[label_name] ||
        opera['operation'] == 'measure'
      )
        return false
    }
    return true
  ...
  getEvoMatrix(label_id) {
    ...
    for (let i = 0; i < deep_length; i++) {
      ...
      for (let j = 0; j < deep_length; j++) {
        ...
        if (stru['bases'][j]['magnitude'] != 0) gate_mats[i][j]['used'] = true
        else gate_mats[i][j]['used'] = false
      }
    }
    ...
    for (let i = 0; i < deep_length; i++) {
      for (let j = 0; j < deep_length; j++) {
        let polar = getExp(all_gate.data[i][j])
        if (max == 0) gate_mats[i][j]['ratio'] = 0
        else gate_mats[i][j]['ratio'] = polar['r'] / max
  ...
  isSparse(label_id, threshold = 1.3, precision = 1e-5) {
    ...
    if (count > threshold * matrix.length) return false
    else return true
  }
  ...
  transferSankeyOrdered(
    ...
    if (per) res['permute'] = parray
    else res['permute'] = range(0, matrix.length)
    res['sankey'] = sankey
    ...
    if (filter_unused) {
      ...
      output_state.bases = output_state.bases.filter(
        (elm) => output_used[elm.id]
      )
    }
    // debugger
    return res
  ...
  _postProcess(state) {
    let max_magn =
      state['input_state']['max_magn'] > state['output_state']['max_magn']
        ? state['input_state']['max_magn']
        : state['output_state']['max_magn']
    ...
    for (let key in state) {
      for (let i = 0; i < state[key]['bases'].length; i++) {
        if (max_magn != 0) {
          for (
            let j = 0;
            j < state[key]['bases'][i]['related_bases'].length;
            j++
          ) {
            state[key]['bases'][i]['related_bases'][j]['ratio'] =
              state[key]['bases'][i]['related_bases'][j]['magnitude'] / max_magn
          }
          state[key]['bases'][i]['ratio'] =
            state[key]['bases'][i]['magnitude'] / max_magn
        }
      }
    }
...
class QInt {
  ...
  parseBinaryQubits(binary_qubits) {
    if (binary_qubits !== undefined) {
      const qubits = binary2qubit1(binary_qubits).map(
        (qubit) => qubit + this.index[0]
      )
      return qubits
    } else {
  ...
  add(value, condition = undefined) {
    ...
    const v_start_qubit = this.index[0],
      v_end_qubit = this.index[this.index.length - 1]
    ...
    if (typeof value === 'number') {
      ...
      value_qubits.forEach((value_qubit, value_index) => {
        let self_qubits_involved = self_qubits.filter(
          (self_qubit, self_index) => {
        ...
        self_qubits_involved.forEach((self_qubit, self_index) => {
          ...
          let controls = [
            ...self_qubits_involved.filter((elm) => elm < self_qubit),
            value_qubit,
          ]
  ...
  subtract(value, condition = undefined) {
    ...
    const v_start_qubit = this.index[0],
      v_end_qubit = this.index[this.index.length - 1] // TODO: 封装成属性
  ...
  invQFT() {
    ...
    for (
      let start = index[0], end = index[1] - 1;
      start < end;
      start++, end--
    ) {
      // debugger
  ...
  Grover(conditional_bits = undefined) {
    let { qc, index } = this
    //let qubits = range(...index)
    if (conditional_bits == undefined) conditional_bits = this.bits()
    this.had()
    this.not()
