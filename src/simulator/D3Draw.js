...
import {
  ...
  toDocs,
} from '../resource/js/quantum-circuit.min'
...
export default class d3Draw {
  ...
  exportD3SVG(data) {
    ...
    const svgWidth =
      (row + this.scaleNum) * this.svgItemWidth > 1299
        ? (row + this.scaleNum) * this.svgItemWidth
        : 1299
    // 设置SVG宽高 高度整体下移了一行
    svg.attr('width', svgWidth * 1.5)
    svg.attr('height', (col + 1) * this.svgItemHeight * 1.5)
    ...
    for (let i = 0; i < data.labels.length; i++) {
      if (data.labels[i].text && data.labels[i].end_operation !== undefined) {
        ...
        if (obj.down_qubit !== Infinity && obj.up_qubit !== Infinity) {
          const lineCol =
            data.labels[i].end_operation - data.labels[i].start_operation
          const labelRow = obj.down_qubit - obj.up_qubit
          this.drawLabel(
            labelG,
            (this.svgItemWidth + this.gate_offest) *
              data.labels[i].start_operation +
              this.labelTranslate,
    ...
    for (const key in data.name2index) {
      for (let i = 0; i < data.name2index[key].length; i++) {
        const lineNum =
          data.name2index[key][data.name2index[key].length - 1] -
          data.name2index[key][0]
        this.drawQint(
          drawG,
          this.svgItemWidth * 2,
  ...
  drawLabel(svg, x, y, width, height, labelText, labelId, isBrushed) {
    ...
    if (isBrushed) {
      ...
      const textG = parentG
        .append('g')
        .attr('transform', `translate(${width / 2},${height + 7}) scale(0.6)`)
      textG
        .append('circle')
        .attr('cx', 0)
        .attr('cy', 0)
      ...
      const textG = parentG
        .append('g')
        .attr('transform', `translate(${width / 2},${height + 7}) scale(0.8)`)
      textG
        .append('text')
        .attr('x', 0)
        .attr('y', 5)
  ...
  drawCselectLine(svg, x, y, height, index, data) {
    ...
    parentG
      ...
      .on('mouseover', function (e) {
        d3.selectAll('.no_click')
          .attr('stroke', 'transparent')
          .attr('fill', 'transparent')
        d3.select(this)
          .select('.no_click')
          .attr('fill', 'rgb(149, 143, 143,0.5)')
      })
  ...
  brushedFn(svg, brushG, labelG) {
    ...
    let brushed_end = (event) => {
      ...
      if (selection) {
        ...
        let operation_notations = svg
          .selectAll('.operation_g')
          .filter((elm) => {
            const { x } = elm
            return x <= x1 && x > x0
          })
        if (operation_notations.data().length) {
          ...
          const labelObj = self.qc.createlabel(
            start_operation,
            end_operation + 1
          )
          this.drawLabel(
  ...
  radianToAngle(radian) {
    if (String(radian) !== '0' && !Number(radian)) {
      const num1 = Number(radian.split('/')[1])
        ? Number(radian.split('/')[1])
        : 1
      const num2 = Number(radian.split('/')[0].split('p')[0])
        ? Number(radian.split('/')[0].split('p')[0])
        : 1
      return (
        _.round(
          _.divide(
            _.multiply(_.divide(_.multiply(num2, Math.PI), num1), 180),
            Math.PI
          )
        ) + '°'
      )
        ...
        if (gates[i][j]) {
          ...
          switch (name) {
              ...
              if (connector > 0) {
                ...
                if (operation.connector === maxConnector) {
                  swapG
                    ...
                    .attr(
                      'y',
                      this.svgItemHeight * (min + 2) - this.svgItemHeight / 2
                    )
                  this.drawLine(
                    swapG,
                    this.operationX(j),
              ...
              if (connector === 0) {
                ...
                if (operation.connector === maxConnector) {
                  ccphaseG
                    ...
                    .attr(
                      'y',
                      this.svgItemHeight * (cu1min + 2) -
                        this.svgItemHeight / 2 +
                        3
                    )
                    .classed('svgtext', true)
                    .append('tspan')
                  ...
                  ccphaseG
                    ...
                    .attr(
                      'y',
                      this.svgItemHeight * (cu1min + 2) - this.svgItemHeight / 2
                    )
                  this.drawLine(
                    ccphaseG,
                    this.operationX(j),
              ...
              if (connector === 0) {
                ...
                if (operation.connector === maxConnector) {
                  ...
                  cryG
                    ...
                    .attr(
                      'y',
                      this.svgItemHeight * (crymin + 2) - this.svgItemHeight / 2
                    )
                  this.drawLine(
                    cryG,
                    this.operationX(j),
            ...
            case 'x':
              ...
              const xParentG = this.drawNameCircle(
                xG,
                this.operationX(j),
                this.operationY(i),
                'x'
            ...
            case 'y':
              ...
              const yParentG = this.drawNameCircle(
                yG,
                this.operationX(j),
                this.operationY(i),
                'y'
            ...
            case 'z':
              ...
              const zParentG = this.drawNameCircle(
                zG,
                this.operationX(j),
                this.operationY(i),
                'z'
            ...
            case 'srndg':
              ...
              const srndgParentG = this.drawSrndg(
                srndgG,
                this.operationX(j),
                this.operationY(i)
              )
            ...
            case 'r2':
              ...
              const r2ParentG = this.drawRnumber(
                r2G,
                this.operationX(j),
                this.operationY(i),
                'Zπ/2'
            ...
            case 'r4':
              ...
              const r4ParentG = this.drawRnumber(
                r4G,
                this.operationX(j),
                this.operationY(i),
                'Zπ/4'
            ...
            case 'r8':
              ...
              const r8ParentG = this.drawRnumber(
                r8G,
                this.operationX(j),
                this.operationY(i),
                'Zπ/8'
            ...
            case 'u2':
              ...
              const u2ParentG = this.drawUnumber(
                u2G,
                this.operationX(j),
                this.operationY(i),
                'u2'
            ...
            case 'u3':
              ...
              const u3ParentG = this.drawUnumber(
                u3G,
                this.operationX(j),
                this.operationY(i),
                'u3'
            ...
            case 's':
              ...
              const sParentG = this.drawNameCircle(
                sG,
                this.operationX(j),
                this.operationY(i),
                's'
            ...
            case 't':
              ...
              const tParentG = this.drawNameCircle(
                tG,
                this.operationX(j),
                this.operationY(i),
                't'
            ...
            case 'sdg':
              ...
              const sdegParentG = this.drawSDg(
                sdegG,
                this.operationX(j),
                this.operationY(i),
                's'
              ...
              if (connector === 0) {
                ...
                if (operation.connector === maxConnector) {
                  ...
                  zzG
                    ...
                    .attr(
                      'y',
                      this.svgItemHeight * (zzmin + 2) - this.svgItemHeight / 2
                    )
                  this.drawLine(
                    zzG,
                    this.operationX(j),
              ...
              if (connector === 0) {
                ...
                if (operation.connector === maxConnector) {
                  cr2G
                    ...
                    .attr(
                      'y',
                      this.svgItemHeight * (cr2min + 2) - this.svgItemHeight / 2
                    )
                  this.drawLine(
                    cr2G,
                    this.operationX(j),
              ...
              if (connector === 0) {
                ...
                if (operation.connector === maxConnector) {
                  cr4G
                    ...
                    .attr(
                      'y',
                      this.svgItemHeight * (cr4min + 2) - this.svgItemHeight / 2
                    )
                  this.drawLine(
                    cr4G,
                    this.operationX(j),
              ...
              if (connector === 0) {
                ...
                if (operation.connector === maxConnector) {
                  cr8G
                    ...
                    .attr(
                      'y',
                      this.svgItemHeight * (cr8min + 2) - this.svgItemHeight / 2
                    )
                  this.drawLine(
                    cr8G,
                    this.operationX(j),
              ...
              if (connector === 0) {
                ...
                if (operation.connector === maxConnector) {
                  ...
                  crxG
                    ...
                    .attr(
                      'y',
                      this.svgItemHeight * (crxmin + 2) - this.svgItemHeight / 2
                    )
                  this.drawLine(
                    crxG,
                    this.operationX(j),
              ...
              if (connector === 0) {
                ...
                if (operation.connector === maxConnector) {
                  ...
                  crzG
                    ...
                    .attr(
                      'y',
                      this.svgItemHeight * (crzmin + 2) - this.svgItemHeight / 2
                    )
                  this.drawLine(
                    crzG,
                    this.operationX(j),
            ...
            case 'cu1':
              ...
              if (connector === 0) {
                ...
                if (operation.connector === maxConnector) {
                  ...
                  this.drawNameCirclethreeLength(
                    cu1G,
                    this.operationX(j),
                    this.operationY(i),
                    'cu1'
              ...
              if (connector === 0) {
                ...
                if (operation.connector === maxConnector) {
                  ...
                  cu2G
                    ...
                    .attr(
                      'y',
                      this.svgItemHeight * (cu2min + 2) - this.svgItemHeight / 2
                    )
                  this.drawLine(
                    cu2G,
                    this.operationX(j),
              ...
              if (connector === 0) {
                ...
                if (operation.connector === maxConnector) {
                  ...
                  cu3G
                    ...
                    .attr(
                      'y',
                      this.svgItemHeight * (cu3min + 2) - this.svgItemHeight / 2
                    )
                  this.drawLine(
                    cu3G,
                    this.operationX(j),
            ...
            case 'reset':
              ...
              const resetParentG = this.drawNameCirclethreeLength(
                resetG,
                this.operationX(j),
                this.operationY(i),
                'rst',
            ...
            case 'measure':
              ...
              const measureParentG = this.drawRead(
                measureG,
                this.operationX(j),
                this.operationY(i)
              )
  ...
  drawLineChart(row, svgWidth) {
    ...
    const scaleX = d3
      ...
      .range([
        this.firstX + this.svgItemWidth / 2,
        (row + 3) * this.svgItemWidth + this.svgItemWidth / 2,
      ])
  ...
  drawStackedBar(data, j, index) {
    ...
    for (const key in data) {
      ...
      const width =
        barWidth * dataArr.length + config.margins.left + config.margins.right
      widthArr.push(width + (j ? widthArr[j - 1] : 0))
      const g = chart_svg
        .append('g')
        .attr(
          'transform',
          `translate(${j ? widthArr[j - 1] : 0},${6 * heightStep})`
        )
  ...
  StackedBarChart(
    ...
    chart.scaleY = d3
      ...
      .domain([
        0,
        d3.max([...data.map((d) => d.magn), ...data.map((d) => d.prob)]),
      ])
      ...
      chart
        ...
        .attr(
          ...
          'translate(' +
            chart.bodyX() +
            ',' +
            (chart.bodyY() + chart.getBodyHeight() / 2) +
            ')'
        )
        .attr('class', 'xAxis')
        .call(customXAxis)
      ...
      chart
        ...
        .attr(
          ...
          'translate(' +
            chart.bodyX() +
            ',' +
            (chart.bodyY() + chart.getBodyHeight() / 2 + 22) +
            ')'
        )
    ...
    chart.addMouseOn = function () {
      g.selectAll('.magn_bar')
        .on('mouseover', function (e, d) {
          ...
          const tipG = d3
            ...
            .attr(
              'transform',
              `translate(${position[0] + 55 + left},${
                position[1] + heightStep * 6 - 10
              })`
            )
          tipG
            .append('rect')
      ...
      g.selectAll('.phase_bar')
        .on('mouseover', function (e, d) {
          const phaseLength = String(_.round(d.phase, 2))
            .split('.')
            .join('').length
    ...
    let brushed_end = (event) => {
      ...
      if (selection) {
        ...
        if (bars.data().length) {
          ...
          const filterResult = self.qc.getIndex(
            index,
            JSON.parse(JSON.stringify(self.filter))
          )
          ...
          // 更新C视图上半
          const barData = self.qc.getVarState(
            index,
            JSON.parse(JSON.stringify(self.filter))
          )
  ...
  cDownStackedBarChart(data, g) {
    ...
    chart.scaleY = d3
      ...
      .domain([
        0,
        d3.max([...data.map((d) => d.magns), ...data.map((d) => d.probs)]),
      ])
      .range([chart.getBodyHeight() / 2, 0])
    // phases Y轴
    chart.scaleY2 = d3
    ...
    function customXAxis(g) {
      ...
      g.selectAll('.tick:nth-last-of-type(1)')
        ...
        .attr(
          'width',
          data[g.selectAll('.tick')._groups[0].length - 1].base.length * 15
        )
        .attr('height', 24)
        .attr('style', 'color:rgba(0,0,0,1)')
        .attr('transform', 'scale(0.7) rotate(45)')
        ...
        .html(
          z.serializeToString(
            getDirac(data[g.selectAll('.tick')._groups[0].length - 1].base)
          )
        )
      const context = d3.path()
      // 自定义X轴线
    ...
    function customYAxis2(g) {
      ...
      g.select('.domain').remove()
      g.selectAll('.tick line').remove()
      g.select('.phaseYAxis .tick:nth-of-type(1)').attr(
        'transform',
        'translate(0,5)'
      )
    }
    ...
    chart.renderMagnsBars = function () {
      ...
      bars
        ...
        .attr('x', (d) => chart.scaleX(d.index))
        .attr('y', (d) =>
          chart.scaleY(d.magns) - 1 > 0 ? chart.scaleY(d.magns) - 1 : 0
        )
        .attr('width', chart.scaleX.bandwidth())
        .attr(
          'height',
          (d) => chart.getBodyHeight() / 2 - chart.scaleY(d.magns)
        )
        .attr('fill', chart._colors[0])
      // .attr('stroke', '#000')
      // .attr('stroke-width', 0.5)
      ...
      g.selectAll('.magns_bar')
        .on('mouseover', function (e, d) {
          ...
          if (position[0] + String(d.base).length * 10 + 96 > 300) {
            tipG = g
              ...
              .attr(
                'transform',
                `translate(${position[0] - String(d.base).length * 10 - 70},${
                  position[1] - 5
                })`
              )
          } else {
            tipG = g
              ...
              .attr(
                'transform',
                `translate(${position[0] + 85},${position[1] - 5})`
              )
          }
        ...
        .on('mousemove', function (e, d) {
          ...
          if (position[0] + String(d.base).length * 10 + 96 > 300) {
            ...
            g.select('.tip').attr(
              'transform',
              `translate(${position[0] + 55},${position[1] - 5})`
            )
          }
        })
        ...
        .on('mouseover', function (e, d) {
          ...
          if (position[0] + String(d.base).length * 10 + 77 > 300) {
            tipG = g
              ...
              .attr(
                'transform',
                `translate(${position[0] - String(d.base).length * 10 - 50},${
                  position[1] - 5
                })`
              )
          } else {
        ...
        .on('mouseover', function (e, d) {
          ...
          if (position[0] + String(d.base).length * 10 + 60 > 300) {
            tipG = g
              ...
              .attr(
                'transform',
                `translate(${position[0] - String(d.base).length * 10 - 30},${
                  position[1] - 5
                })`
              )
          } else {
    ...
    chart.addZoom = function () {
      ...
      function zoomed(event) {
        chart.scaleX.range(
          [0, chart.getBodyWidth()].map((d) => event.transform.applyX(d))
        )
        chart.scaleX2.range(
          [0, chart.getBodyWidth()].map((d) => event.transform.applyX(d))
        )
        ...
        if (event.transform.k > 5.28) {
          ...
          chart
            .svg()
            .selectAll('.xAxis .tick foreignObject')
            .attr('style', 'color:rgba(0,0,0,0)')
          chart
            .svg()
            .select('.xAxis')
  ...
  drawDinput(
    ...
    childG
      ...
      .attr(
        'transform',
        `translate(${(20 * (1 - inWidth)) / 2},${(20 * (1 - inWidth)) / 2})`
      )
      .append('rect')
    ...
    if (isNeedShowData) {
      ...
      childG.on('mouseover', function (e) {
        ...
        // 碰撞检测
        let initTop = offsetY
          ? offsetY - scrollTop + 40
          : e.offsetY - scrollTop + 36
        let initLeft = offsetX
          ? offsetX + 50 - scrollLeft
          : e.offsetX - scrollLeft + 10
        const initHeight = 32 * (allKeys.length / 2)
        if (
          initTop + initHeight + chartDivOffsetTop >
          drawDiv._groups[0][0].clientHeight
        ) {
          initTop = offsetY
            ? offsetY - scrollTop - 40
            : e.offsetY - scrollTop - 36
        ...
        if (
          initLeft + 200 + chartDivOffsetLeft >
          drawDiv._groups[0][0].clientWidth
        ) {
          initLeft = offsetX
            ? offsetX - 220 - scrollLeft
            : e.offsetX - scrollLeft - 230
        }
        const showDataDiv = chartDiv
          ...
          .attr(
            'style',
            `height:${initHeight}px;top:${initTop}px;left:${initLeft}px;border:1px solid black`
          )
  ...
  drawDCircle(
    ...
    if (arcDeg) {
      ...
      // 用浅色圆填充剩余的部分
      const opacityCircleR = {
        startAngle: (Math.PI * arcDeg) / 180,
      ...
      if (arcR < 1 && arcR > 0) {
        const borderCircleR = {
          startAngle: (Math.PI * (arcDeg - 1)) / 180,
          endAngle: (Math.PI * arcDeg) / 180,
        }
        const borderPath = d3
          .arc()
          .innerRadius(0)
      ...
      for (let i = 0; i < index.length; i++) {
        if (index[i][1] - index[i][0] > 2) {
          ...
          parentG
            ...
            .attr(
              'y',
              (this.dLength * (index[i][1] - index[i][0] + 1)) / 2 - 12
            )
            .append('xhtml:div')
            .attr('height', '100%')
            .attr('width', '100%')
    ...
    childG
      ...
      .on('mouseover', function (e) {
        ...
        const relaedDiv = chartDiv
          ...
          .attr(
            'style',
            `top:${e.offsetY - scrollTop + 36}px;left:${
              e.offsetX - scrollLeft + 10
            }px;height:${self.dLength * data.length + 17}px;width:${
              self.dLength + 8
  ...
  drawSankey(data) {
    ...
    const { input_state: inputStateData, output_state: outStateData } =
      this.qc.getState(data.id)
    ...
    // 计算圆圈g X轴向右移动的距离
    const circleGtransformX =
      (inputStateData.vars.length + 4) * this.dLength + 14
    // 计算输入input X轴移动
    const inputGTransformX =
      (inputStateData.vars.length + 1) * this.dLength + 14
    // 计算out_input X轴移动
    const outGTransformX = (inputStateData.vars.length + 7) * this.dLength + 14
    // 计算out_input 浅色块X轴移动
    const outRelatedGX =
      outGTransformX + (outStateData.vars.length + 1) * this.dLength
    // 设置svg的宽高
    ...
    for (let i = 0; i < sankeyData.length; i++) {
      const color = sankeyData[i].used
        ? this.dCircleUsedColor
        : this.dCircleColor
    ...
    for (let i = 0; i < inputStateData.vars.length; i++) {
      ...
      const textG = svg
        ...
        .attr(
          'transform',
          `translate(${this.dLength * (i + 1) + 14},${this.dLength})`
        )
      const qNameX = this.dLength * (i + 1) + 14
      const qNameG = svg
        .append('g')
        .classed('q_name_g', true)
        .attr('transform', `translate(${qNameX + 5},${i * 10})`)
    ...
    for (let i = 0; i < outStateData.vars.length; i++) {
      ...
      const textG = svg
        ...
        .attr(
          'transform',
          `translate(${outGTransformX + this.dLength * (i + 1)},${
            this.dLength
          })`
        )
    ...
    const drawOutRelaedNumG = svg
      ...
      .attr(
        'transform',
        `translate(${outRelatedGX + this.dLength},${this.dLength})`
      )
    for (let j = 0; j < outBases.length; j++) {
      this.drawDinput(
        outG,
  ...
  drawMatrix(data) {
    ...
    // 计算矩阵g Y轴向下移动的距离
    const circleGtransformY =
      (inputStateData.vars.length + 2) * this.dLength + 14

    // 计算输入input Y轴移动
    const inputGTransformY =
      (inputStateData.vars.length + 1) * this.dLength + 14
    // 计算out_input X轴移动
    const inputWidth = (inputStateData.bases.length + 1) * this.dLength + 15
    // 计算out_input 浅色块X轴移动
    const outRelatedGX =
      inputWidth + (outStateData.vars.length + 1) * this.dLength
    // 绘制矩阵
    // 设置svg的宽高
    const svgHeight =
      circleGtransformY + outStateData.bases.length * this.dLength
    ...
    for (let i = 0; i < circleData.length; i++) {
      for (let j = 0; j < circleData[i].length; j++) {
        const color = circleData[i][j].used
          ? this.dCircleUsedColor
          : this.dCircleColor
    ...
    for (let i = 0; i < outStateData.vars.length; i++) {
      ...
      // 绘制text |0>
      for (let j = 0; j < outStateData.bases.length; j++) {
        outVar2ValueArr.push(
          outStateData.bases[j].var2value[outStateData.vars[i]]
        )
      }
      // 得到重复的开始/结束位置:{10:[[0,1],[4,8]]}
    ...
    const drawOutRelaedNumG = svg
      ...
      .attr(
        'transform',
        `translate(${outRelatedGX + this.dLength},${circleGtransformY})`
      )
    for (let j = 0; j < outStateData.bases.length; j++) {
      this.drawDinput(
        outG,
    ...
    for (let i = 0; i < inputStateData.vars.length; i++) {
      ...
      const textG = svg
        ...
        .attr(
          'transform',
          `translate(${this.dLength + 15},${this.dLength * (i + 1) + 14})`
        )
      // 绘制变量名
      ...
      const qNameG = svg
        ...
        .attr(
          'transform',
          `translate(${inputWidth + this.dLength * (j + 1) + 5},${qNameY})`
        )
      this.drawDqName(qNameG, inputStateData.vars[i])
      j--
      // // 绘制文字 |0>
      ...
      // 绘制text |0>
      for (let j = 0; j < inputStateData.bases.length; j++) {
        inputVar2ValueArr.push(
          inputStateData.bases[j].var2value[inputStateData.vars[i]]
        )
      }
