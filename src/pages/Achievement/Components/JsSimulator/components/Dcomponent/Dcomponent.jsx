...
import { getDirac } from '@/utils/Mathjax'
import { labelsStroe } from '@/store/line'
...
const Dcomponent = (props) => {
        ...
        const relaedDiv = chartDiv
          ...
          .attr(
            ...
            `top:${e.offsetY - scrollTop + 36}px;left:${
              e.offsetX - scrollLeft + 10
            }px;height:${dLength * data.length + 17}px;width:${
              dLength + 8
  ...
  const drawSankey = (data) => {
    ...
    // 计算out_input 浅色块X轴移动
    const outRelatedGX =
      outGTransformX + (outStateData.vars.length + 1) * dLength
    // 设置svg的宽高
    ...
    for (let i = 0; i < outStateData.vars.length; i++) {
      ...
      const textG = svg
        ...
        .attr(
          'transform',
          `translate(${outGTransformX + dLength * (i + 1)},${dLength})`
        )
    ...
    const outG = svg
      .append('g')
      .classed('put_g', true)
      .attr('transform', `translate(${outGTransformX},${dLength})`)
  ...
  const drawMatrix = (data) => {
    ...
    for (let i = 0; i < outStateData.vars.length; i++) {
      ...
      const textG = svg
        ...
        .attr(
          'transform',
          `translate(${inputWidth + dLength * (i + 1)},${circleGtransformY})`
        )
    ...
    const drawOutRelaedNumG = svg
      ...
      .attr(
        'transform',
        `translate(${outRelatedGX + dLength},${circleGtransformY})`
      )
    for (let j = 0; j < outStateData.bases.length; j++) {
      drawDinput(
        outG,
    ...
    for (let i = 0; i < inputStateData.vars.length; i++) {
      ...
      const textG = svg
        ...
        .attr(
          'transform',
          `translate(${dLength + 15},${dLength * (i + 1) + 14})`
        )
      // 绘制变量名
      ...
      const qNameG = svg
        ...
        .attr(
          'transform',
          `translate(${inputWidth + dLength * (j + 1) + 5},${qNameY})`
        )
      drawDqName(qNameG, inputStateData.vars[i])
      j--
      // // 绘制文字 |0>
