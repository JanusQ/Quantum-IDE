...
import AnalysisCircuitPage from './components/AnalysisCircuitPage'
...
function App() {
  ...
  const { projectName, projectId } = { projectName: 'test', projectId: '12345' }
  ...
  const [isVQA, setisVQA] = useState(false)
  const drawBarChart = (probs, sample) => {
    if (initOption == 'VQA') {
    }
    if (initOption !== 'VQA') {
  ...
  const realRun = async (qc, sample, runValue) => {
    ...
    try {
      ...
      // drawBarChart(data.probs, sample)
      // console.log(data, 66)
  ...
  const [initOption, setInitOption] = useState('ghz_state')
  // 选择改变编辑器的内容
    ...
    <>
      ...
      <div className="App" style={{ backgroundColor: backcolor }}>
        ...
        <div
          className="right-div"
          style={{
            width: isShowAMode ? 'calc(100% - 500px)' : '100%',
